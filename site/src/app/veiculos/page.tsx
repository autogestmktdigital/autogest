                  ...
                  {filteredVehicles.map((vehicle) => (
                    <Link key={vehicle.id} href={`/veiculos/${vehicle.id}`} className="overflow-hidden rounded-2xl border border-white/10 bg-brothers-gray transition hover:-translate-y-1 hover:border-brothers-green/40">
                      ...
                      <div className="p-5">
                        ...
                        <WhatsAppLink
                          href={`https://wa.me/5511918622241?text=${encodeURIComponent(`Olá! Vi no site o ${vehicle.brand} ${vehicle.model}${vehicle.version ? ` ${vehicle.version}` : ''} ${formatVehicleYear(vehicle)} por ${formatPrice(vehicle.price)} e gostaria de mais informações.`)}`}
